...
/// the same values passed into the [CustomSliverRefreshControl].
///
...
/// A callback function that's invoked when the [CustomSliverRefreshControl] is
/// pulled a `refreshTriggerPullDistance`. Must return a [Future]. Upon
/// completion of the [Future], the [CustomSliverRefreshControl] enters the
...
class CustomSliverRefreshControl extends StatefulWidget {
  ...
  static RefreshIndicatorMode state(BuildContext context) {
    final _CustomSliverRefreshControlState state =
        context.findAncestorStateOfType<_CustomSliverRefreshControlState>()!;
  ...
  static Widget _buildIndicatorForRefreshState(
    ...
    switch (refreshState) {
      ...
      case RefreshIndicatorMode.refresh:
        ...
        return CustomRefreshIndicator(percent: percentageComplete);
      case RefreshIndicatorMode.done:
        // When the user lets go, the standard transition is to shrink the spinner.
        return CustomRefreshIndicator(percent: percentageComplete, done: true);
...
class _CustomRefreshIndicatorState extends State<CustomRefreshIndicator> {
  ...
  void onRivInit(Artboard artboard) {
    ...
    _bump = controller.findInput<bool>('done') as SMITrigger;
    _percent = controller.findInput<double>('rate') as SMINumber;
  }

  void _hit() => _bump?.fire();

  @override
  Widget build(BuildContext context) {
    if (_percent != null) {
      _percent!.value = widget.percent * 100;
    }
    if (widget.done) {
      _hit();
    }
